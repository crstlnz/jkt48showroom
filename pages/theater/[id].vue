  ...
  <div>
    ...
    <LayoutSingleRow v-else :title="`${title} - ${$dayjs(data?.date).locale(locale).format('DD MMMM YYYY')}`">
      <div v-if="data.shows?.length" class="px-4 md:px-5 space-y-10">
        <div v-for="[idx, theater] in data.shows.entries()" :key="theater.id"
          ...
          <div class="flex flex-col gap-5 md:flex-row md:gap-4">
            ...
            <div class="space-y-2 md:p-3">
              ...
              <div class="flex gap-3 items-center">
                ...
                <NuxtImg class="self-center rounded-md" :src="`${$cloudinaryURL}/assets/jkt48${theater.team.img}`"
              ...
              <div class="flex gap-3 justify-end md:items-end flex-col md:flex-row">
                <div v-if="theater.seitansai?.length || theater.graduation?.length" class="flex-1">
                  <div v-if="theater.seitansai?.length" class="flex flex-col gap-1.5">
                    ...
                    <div
                      ...
                      <NuxtImg v-for="member in theater.seitansai" :key="member.id"
                  ...
                  <div v-if="theater.graduation?.length" class="flex flex-col gap-1.5">
                    ...
                    <div
                      class="grid grid-cols-[repeat(auto-fill,minmax(80px,1fr))] md:grid-cols-[repeat(auto-fill,minmax(130px,1fr))] md:flex flex-wrap gap-4 md:gap-5">
                      <NuxtImg v-for="member in theater.graduation" :key="member.id"
                        class="bg-container block aspect-[8/10] w-[100px] md:w-[120px] overflow-hidden rounded-xl object-cover"
                        :src="member.img || pic" alt="Default member picture" fit="fill" :modifiers="{
                          aspectRatio: 8 / 10,
                          gravity: 'faceCenter',
                ...
                <div class="flex flex-col gap-2 w-full md:w-[200px] lg:w-[250px] xl:w-[280px]">
                  <div class="font-semibold mt-2 flex items-center gap-2">
                    <Icon :name="getTheaterState(theater.date) === 'ended' ? 'pepicons-pop:internet' : 'ep:ticket'"
                      class="text-yellow-500" />
                    <span>{{ $t(getTheaterState(theater.date) === 'ended' ? 'official_link' : 'theater_ticket')
                      }}</span>
                  </div>
                  <NuxtLink :to="`https://jkt48.com/theater/schedule/id/${theater.id}?lang=id`" external target="_blank"
                    ...
                    <Icon v-if="getTheaterState(theater.date) !== 'ended'" name="ep:ticket"
                  ...
                  <NuxtLink v-if="theater.showroomTheater && getTheaterState(theater.date) !== 'ended'"
