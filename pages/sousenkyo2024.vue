    ...
    <LayoutSingleRow v-else title="Sousenkyo 2024">
      <template #default>
        <Transition name="fade">
          <SousenkyoVideoDialog
            v-if="openVideo && sousenkyoMember" :key="sousenkyoMember.id" :data="sousenkyoMember"
            @on-dismiss="back"
          />
        </Transition>
        <div>
          ...
          >
            <div v-for="member in members" :key="member.id" class="bg-container rounded-xl p-3 md:p-4">
              <DeferImage
                :alt="member.name" :src="`https://ssk.jkt48.com/2024${member.data?.mobileBg}`"
              ...
              <button
                type="button" class="bg-red-500 px-3 py-1.5 rounded-xl w-full flex items-center justify-start gap-1"
                @click="() => watchVideo(member)"
              >
