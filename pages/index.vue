...
<template>
  <LayoutRow title="Home">
    <template #actionSection>
      <ClientOnly>
        <div v-if="!isXL" class="pointer-events-none inset-x-0 max-sm:absolute xl:hidden">
          <div
            class="bg-container pointer-events-auto float-right flex items-center rounded-2xl p-1.5 text-sm ring-blue-500 focus-within:ring-2 max-sm:mx-3 max-sm:focus-within:w-[calc(100%_-_24px)] max-sm:focus-within:pl-3"
            :class="{ 'pl-3 max-sm:w-[calc(100%_-_24px)]': search.length !== 0 }"
          >
            <input
            ...
            >
            <button
              v-if="search.length === 0" aria-label="Search"
              class="group flex h-7 w-7 items-center justify-center rounded-xl p-1 sm:hover:bg-blue-500"
              @click="searchInput?.focus()"
            >
              <Icon
                name="uil:search"
                class="h-full w-full text-slate-800  dark:text-white/50 dark:group-hover:text-white"
              />
            </button>
            <button
              v-else aria-label="Close"
              class="group flex h-7 w-7 items-center justify-center rounded-xl p-1 sm:hover:bg-blue-500"
              @click="search = ''"
            >
              <Icon
                name="ic:round-close"
                class="h-full w-full text-neutral-400/80 group-hover:text-white dark:text-slate-100"
              />
            </button>
          </div>
        </div>
    ...
    <template #default>
      ...
      <div class="flex flex-col gap-3 md:gap-4">
        ...
        <div class="mx-3 space-y-3 md:mx-4 xl:grid xl:grid-cols-5 xl:grid-rows-1 xl:gap-4 xl:space-y-0">
          <HomeContainer
            class="col-span-2" :title="$t('nextlive')" icon-class="bg-orange-500"
            more="https://campaign.showroom-live.com/akb48_sr/#/" :more-text="$t('showonshowroom')"
            :more-ext-link="true"
          >
            <HomeNextLive />
          </HomeContainer>
    ...
    <template #sidebar>
      <div
        class="max-xl:hidden group bg-container flex items-center gap-4 rounded-full px-4 ring-blue-500 focus-within:ring-2 xl:mt-4"
      >
        ...
        <button
          ...
          class="hidden h-6 w-6 shrink-0 rounded-full bg-blue-500 text-white group-focus-within:block group-hover:block"
          @click="clearSearch"
        >
