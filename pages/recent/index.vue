...
<template>
  <LayoutRow :title="$t(title)" :mobile-side="false">
    ...
    <div class="relative z-10">
      <ClientOnly>
        <template #fallback>
          <div key="loading" class="space-y-2 px-3 md:px-4 md:space-y-4">
            <PulseRecentDetailCard v-for="index in Array(10).keys()" :key="index" />
          </div>
        </template>
        <Transition name="fade-abs">
          <div v-if="pending && (dataset?.data?.length ?? 0) === 0" key="loading" class="space-y-2 px-3 md:px-4 md:space-y-4">
            <PulseRecentDetailCard v-for="index in Array(10).keys()" :key="index" />
          </div>
          <div
