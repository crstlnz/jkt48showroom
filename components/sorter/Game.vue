...
import { decimalFormat } from '~~/library/utils'
...
const { start, state, stop, GameState, cardOne, cardTwo, pick, result, reset, undo, progress } = useMemberSorter()
watch(state, (val) => {
    ...
    <Transition name="gamecontrol" mode="out-in">
      ...
      >
        <div class="flex w-full items-center justify-evenly">
          ...
          <SorterCard :flip="state !== GameState.STARTED" class="reverse w-40 md:w-44 lg:w-60" :member="cardTwo" @click="() => pick('two')" />
        </div>
        <Transition name="gamecontrol" mode="out-in">
          <SorterFilterMember
