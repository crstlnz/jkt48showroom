...
<template>
  <NuxtLink :to="`/theater/${theater.url}`" class="bg-container p-3 md:p-4 rounded-xl flex flex-col gap-3">
    <div class="rounded-xl bg-black/10  w-full relative">
      <div v-if="theaterState === 'ongoing'" class="absolute flex items-center gap-2 top-0 left-0 m-3 bg-red-500 font-semibold px-2.5 py-1 rounded-xl border border-black/20">
      ...
      <div v-else-if="$dayjs(theater.date).isToday()" class="absolute top-0 left-0 m-3 bg-red-500 font-semibold text-white px-2.5 py-1 rounded-xl border border-black/20">
        {{ $t('today') }}
      </div>
      <div v-else-if="$dayjs(theater.date).isTomorrow()" class="absolute top-0 left-0 m-3 bg-orange-500 font-semibold text-white px-2.5 py-1 rounded-xl border border-black/20">
        {{ $t('tomorrow') }}
      </div>
      <NuxtImg
