...
<template>
  <div class="flex w-full flex-col gap-3">
    <div v-for="form in formShowroom" :key="form.id" class="flex gap-3">
      ...
      <Component :is="getForm(form.component ?? 'text')" v-model="form.data" :keys="(form as any).keys" :data="form.options || []" form-id="start" :placeholder="form.placeholder ?? form.title" input-class="bg-container-2 flex-1 text-sm md:text-base" class="w-0 flex-1" />
    </div>
