...
async function getNextLive(): Promise<INextLive[]> {
  return await cache.fetch<INextLive[]>("next_live", getNextLiveCookies, time).catch(() => []);
}
...
async function fetchNextLive(membersData: IMember[] = null): Promise<INextLive[]> {
  try {
    const members: IMember[] = (membersData ?? (await getMembers())).filter((i) => i.room_exists);
    const promises: Promise<INextLive | null>[] = [];
    for (const member of members) {
      promises.push(
        (async (): Promise<INextLive> => {
          try {
            const response = await fetch(config.nextLiveURL(member.room_id));
    ...
    let data: INextLive[] = (await Promise.all(promises)).filter((i) => i);
