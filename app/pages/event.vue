...
<template>
  <LayoutRow :title="$t('menu.event')">
    <div v-if="pending" key="pending" class="mx-3 md:mx-4 hidden">
    ...
    <div v-else-if="error" key="error" class="flex justify-center items-center pb-4 aspect-video">
      <div class="flex flex-col items-center">
        <Image :src="`${$imgCDN}/assets/svg/web/error.svg`" sizes="320px" fit="fill" alt="" class="p-5" />
        {{ $t("error.unknown") }}
      </div>
    </div>
    <div v-else-if="data" key="data" class="mx-3 md:mx-4">
      <div class="text-xl sm:text-2xl font-bold mb-2 flex gap-2 items-center">
        <Icon name="mdi:calendar-clock" class="text-blue-500" />
        <span>
          {{ $t('upcoming_theater') }}
      ...
      <div v-if="data?.theater?.upcoming?.length" key="upcoming" class="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
