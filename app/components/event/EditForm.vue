  ...
  return [
    ...
    },
    {
      ...
      options: (props.jkt48members ?? []).map((i) => {
        const nick = (i.nicknames?.length ?? 0) > 0 ? ` (${i.nicknames?.[0]})` : ''
        return {
          title: `${i.name}${nick}`,
          value: i.id,
...
<template>
  <div class="flex flex-col gap-3">
    ...
    <div v-for="form in formTheater" :key="form.id" class="flex gap-3">
      <div class="w-16.25 shrink-0 truncate pt-1.5 md:w-22.5 lg:w-30 text-sm md:text-base" :class="{ 'leading-11.25!': isMultiple(form.component) }">
        {{ form.title }}
      </div>
