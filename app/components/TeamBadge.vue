...
<template>
  <div v-tooltip="$t('tooltip.show_by_team', { team: _team })" class="flex gap-1.5">
    <div v-if="type !== 'text'" class="bg-white size-6 flex items-center justify-center rounded-md" :style="{ border: `2px solid ${teamColor.bg}` }">
      <Image :src="teamColor.icon" sizes="16px" />
    </div>
    <span
      v-if="['passion', 'love', 'dream'].includes(teamKey ?? '')"
      :style="{
    ...
    >
      <span v-if="type === 'text'" class="font-sans! pb-0.5">Team</span>
