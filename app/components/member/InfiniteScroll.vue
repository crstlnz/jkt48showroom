...
const recentHeight = computed(() => {
  if (isXL.value) return 210
  if (isLarge.value) return 190
  if (isMedium.value) return 173
  if (isSmall.value) return 147
  return 129
})
...
<template>
  <div ref="infiniteScroll" class="flex flex-col gap-3 px-3 md:px-4">
    ...
    <div v-if="error" class="bg-container flex aspect-video w-full flex-col items-center justify-center gap-10 rounded-xl">
      <Image class="mx-auto w-80 max-w-[80%]" :src="`${$imgCDN}/assets/svg/web/error.svg`" sizes="320px" fit="fill" />
    ...
    <div v-else class="wew">
      <VirtualView :items="dataset" :estimate-size="() => recentHeight" :gap="16">
  ...
  >
    <div
      v-if="!isTop"
      class="fixed bottom-20 left-1/2 z-aboveNav flex w-[180px] max-w-[80%] -translate-x-1/2 overflow-hidden rounded-xl bg-second-2/95 font-bold text-white transition sm:bottom-10"
    >
      <button
        type="button"
